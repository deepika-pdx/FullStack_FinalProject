...
const UpcomingTodoItem = (props) => {
    ...
    const renderUpdateFormup = () => (
        <form className="update-form" onSubmit={(e) => { updateItemup(e, !open); }}>
          <input className="update-new-input" type="text" placeholder="New Item"
            onChange={(e) => { setUpdateItemTextup(e.target.value);}} value={updateItemTextup}/>
          <DatePicker name="Select a date" format="MM-dd-y" selected={updateDate}
            onChange={(date) => setupdateDate(date)} isClearable popperClassName="some-custom-class"
      ...
      const updateItemup = async (e, open) => {
        ...
        try {
          ...
          if (updateItemTextup.length !== 0 && dateup.length !== 0) {
            ...
            const updatedItemIndex = props.listItemsup.findIndex((item) => item._id === isUpdatingup);
                ...
                <div className="modal">
                    ...
                    <div className="content">
                        <div className="todo-listItems">
                            {Array.isArray(props.listItemsup) ? props.listItemsup.map((item, index) => (
                                    <div className={index % 2 === 0 ? "bck-blue" : "bck-white"}>
                                        <div className="todo-item"> {isUpdatingup === item._id ? ( renderUpdateFormup()) : (
                                            <>
